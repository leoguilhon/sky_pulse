...
Search is another projection of the shared aircraft snapshot, independent of the current viewport. The existing authenticated endpoint accepts a validated callsign/ICAO24 substring, ranks exact matches first, excludes expired observations and limits results to 20 before the catalog join. It does not look up routes for search results or index ephemeral route-reference IATA flight numbers. Existing provider caching, cooldowns and session protection remain in force.

Route normalization now retains validated latitude/longitude values from adsbdb. The browser shares one route contract between the inspector and renderer. A stable callback publishes route results and clears them on lookup replacement, selection changes or unmount. Renderer recreation restores the current route. One independent GeoJSON source carries bounded great-circle reference legs and airport points; it is updated only when the route changes, independently of aircraft animation. Airport labels use the existing glyph host and collision placement. No new external service or persistent route database is added.

Each consecutive airport pair defines a great-circle leg with 128 intervals. Date-line crossings are split at the boundary; coincident and antipodal endpoints omit a degenerate or ambiguous line. Unknown coordinates omit only adjacent legs and preserve airport text. Airport navigation retains the selected observation outside the local feed until the existing ten-minute expiry, without advancing its timestamp; ordinary geographic navigation keeps Phase 7 selection behavior unless the user is already previewing route airports. The UI reports loaded aircraft to include this inspection context.

The phase includes route-airport markers/details/navigation, not a standalone airport directory. Historical routes, schedules, arrivals/departures and globally indexed IATA flight-number searches require separate data coverage. Deterministic tests cover search validation/authentication/cache reuse, missing/expired matches, multi-leg and date-line geometry, offscreen result selection, airport navigation, mobile layout and existing movement/expiry behavior.
