...
A pure client motion model tracks each ICAO24 observation and its displayed start position. Newer timestamps create a 30-second transition; equal timestamps only refresh metadata, and older timestamps retain the existing target. Interruption starts from the current interpolated position. Longitude and heading use shortest-angle interpolation. There is no extrapolation from speed or heading, so shared provider caching never creates fictional ongoing motion.

The existing request loop remains sequential with 30-second scheduling after completion, abort on cleanup, and authentication failure handling. A separate one-second expiry sweep removes positions 10 minutes after observation even during API outages, synchronizing map, count, picker and selection. The threshold accommodates the default five-minute provider cache while limiting old markers independently of the server fallback window.

MapLibre receives one batched GeoJSON update at up to 30 Hz during transitions. Animation stops when targets are reached, pauses in hidden tabs, honors reduced motion, and is canceled on renderer disposal. Aircraft source loading events do not mark geographic tiles as loading. React and the inspector retain reported observations rather than rerendering on every animation frame.

Deterministic unit tests cover angular wrap, transition interruption, cached/older observations, expiry, reappearance, missing headings and reduced motion. The isolated browser test also verifies periodic refresh and selected-aircraft expiry during a feed failure.
