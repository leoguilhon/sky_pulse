...
2. Set `POSTGRES_PASSWORD` to a unique local password and add the OpenSky OAuth `OPENSKY_CLIENT_ID` and `OPENSKY_CLIENT_SECRET` from your account page. Enclose values containing literal `$` or `#` in single quotes in `.env`. Never commit this file.
...
| `AVIATION_SCOPE`                              | `worldwide`                   | Feed coverage: `sao_paulo` (5 × 5 degrees) or `worldwide`.                                                          |
| `AVIATION_CACHE_TTL_SECONDS`                  | `120`                         | Shared worldwide snapshot lifetime (10-3600 seconds). The default uses 2,880 OpenSky state credits per day.         |
| `OPENSKY_CLIENT_ID` / `OPENSKY_CLIENT_SECRET` | Scope-dependent               | OpenSky OAuth client credentials; both are required for `worldwide`.                                                |
...
The map uses MapLibre GL JS with a bundled Natural Earth image for complete global coverage, loaded once and retained during rotation and zoom. Regional and street detail uses the OpenFreeMap Dark vector style from zoom 4 onward; the global base remains visible while those tiles load. OpenFreeMap serves OpenStreetMap/OpenMapTiles tiles, fonts, and symbols directly to the browser. These details and place labels require internet access; no map API key is needed. Place names prefer English where available and otherwise retain source geographic names. Attribution stays visible, and data sources, regeneration instructions and style licenses are included in `apps/web/public/maps/`.
...
The authenticated `GET /api/aircraft` endpoint loads either a 5 × 5 degree São Paulo snapshot or a worldwide snapshot, selected with `AVIATION_SCOPE=sao_paulo|worldwide`. Aircraft with valid coordinates are normalized into SkyPulse's provider-independent model and rendered as one batched GeoJSON layer. Missing callsigns, altitude, speed, heading, and other optional values are accepted; rows without valid coordinates are omitted.

The browser never contacts OpenSky or receives provider credentials. Worldwide coverage requires OAuth client credentials through `OPENSKY_CLIENT_ID` and `OPENSKY_CLIENT_SECRET`; the server fails fast when either value is missing or only one is configured. São Paulo coverage can run anonymously, but its cache must be at least 216 seconds to remain within the 400-credit daily allowance. OAuth access tokens are cached until shortly before expiry and refreshed once after an unauthorized response.

Snapshots are shared in memory across users for two minutes by default. A worldwide request costs four OpenSky state credits, resulting in 2,880 credits per day on one continuously running API process. Concurrent cache misses share the same request, so page loads do not multiply external calls. Provider `429` cooldowns are honored, and the last snapshot can be served as explicitly stale for up to 15 minutes during a rate limit or temporary outage. A missing or failed feed does not prevent geographic exploration. The browser refreshes after two minutes, matching the shared provider cache and avoiding repeated delivery of large global snapshots. New position observations transition smoothly on the client.
...
The browser polls every two minutes after each request completes, with one request in flight and a 12-second timeout. Each newer aircraft observation transitions from its currently displayed values to the reported values over 30 seconds. Position, altitude, ground speed, heading, and vertical rate stay synchronized during the transition; longitude and heading follow the shortest angular path, including the antimeridian and north crossing. OpenSky's last-contact time controls telemetry freshness independently from its position timestamp, so a speed or altitude update is accepted even when coordinates have not changed. Older observations cannot rewind the rendered track. New aircraft appear at their reported values, and aircraft absent from the next snapshot are removed immediately.

Animation updates the shared GeoJSON source at up to 30 frames per second without per-frame React updates. The selected aircraft's sampled telemetry is sent to the inspector at most four times per second, keeping its measurements synchronized without rerendering React on every map frame. Animation pauses in hidden tabs, resumes at the elapsed transition time, and stops at the last reported values. Reduced-motion preferences display observations immediately. This is interpolation between real observations, not predicted live telemetry: with the default two-minute provider cache, values remain stationary after a transition until a newer observation arrives. The inspector identifies both the latest telemetry contact and, when different, the original position timestamp.
...
The panel shows callsign (ICAO24 fallback), altitude in feet, ground speed in knots, heading / true track in degrees, vertical rate in feet per minute, coordinates, registration country, available model/type information, and UTC telemetry/position timestamps. Flight measurements follow the same interpolated sample as the moving symbol. Missing values are explicitly unavailable and stale snapshots retain their warning. The country is inferred from the transponder address, not the flight's departure country.
